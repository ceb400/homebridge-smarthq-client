...
export class ControlLock {
    ...
    constructor(platform, accessory, deviceServices, deviceId) {
        ...
        if (!this.platform.config.addDwControlLock) {
            // If user has not enabled Control Lock switch, then don't add it
            return;
        }
        //====================================================================================
        // Check to see if the device has any supported Control Lock services
        ...
        if (!hasControlLock) {
            this.client.debug("No supported Control Lock service found for device: " +
                this.accessory.displayName);
            return;
        }
        this.client.debug("Adding Dishwasher Controls Lock Switch");
        //=====================================================================================
        // create a Control Lock switch for the Dishwasher
        //=====================================================================================
        const displayName = "Dw Control Lock";
        const controlsLock = this.accessory.getService(displayName) ||
            this.accessory.addService(this.Service.Switch, displayName, "dw-control-lock-123");
