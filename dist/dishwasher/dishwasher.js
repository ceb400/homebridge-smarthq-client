...
export class Dishwasher {
    ...
    api;
    totalSeconds = 0;
    timeRemainingFromWebSocket = 0;
    energyMeterValuePerHour = 0;
    washZoneMap = new Map();
    washTempMap = new Map();
    heatedDryMap = new Map();
    presetMap = new Map();
    ...
    currentPreset = "cloud.smarthq.domain.dishwasher.normal";
    currentWashTemp = "cloud.smarthq.type.dishwasher.washtemp.none";
    currentWashZone = "cloud.smarthq.type.dishwasher.washzone.both";
    currentHeatedDry = "cloud.smarthq.type.dishwasher.heateddry.none";
    currentbottleWash = false;
    currentSteam = false;
    currentSilverwareWash = false;
    //========  preset mode constants  ========
    NORMAL_MODE = "cloud.smarthq.domain.dishwasher.normal";
    HEAVY_MODE = "cloud.smarthq.domain.dishwasher.heavy";
    AUTOSENSE_MODE = "cloud.smarthq.domain.dishwasher.autosense";
    ONE_HOUR_MODE = "cloud.smarthq.domain.dishwasher.timed.hours.1";
    PLATPLUS_MODE = "cloud.smarthq.domain.dishwasher.brand.cascade.platinumplus";
    RINSE_MODE = "cloud.smarthq.domain.dishwasher.rinse";
    constructor(platform, accessory, deviceServices, deviceId, groupAccessory) {
         ...
         */
        this.client.on("service_update", (message) => {
            ...
            if (message.domainType === "cloud.smarthq.domain.energy" && message.deviceType === "cloud.smarthq.device.dishwasher") {
                ...
                this.energyMeterValuePerHour += message.state?.meterValueDelta || 0; // sum for the hour until reset
            }
            if (message.serviceType === "cloud.smarthq.service.cycletimer" &&
                ...
                if (this.totalSeconds === 0) {
                    this.totalSeconds = message.state?.secondsRemaining || 0;
                }
                this.timeRemainingFromWebSocket =
                    message.state?.secondsRemaining || 0;
                this.client.debug("Wash Modes - Seconds remaining from WebSocket: " +
                    this.timeRemainingFromWebSocket +
                    " Total seconds: " +
                    this.totalSeconds);
                this.getCyclePct();
        ...
        const washTemps = this.getAvailableItemsByType("washTempAvailable");
        const washZones = this.getAvailableItemsByType("washZoneAvailable");
        const heatedDrys = this.getAvailableItemsByType("heatedDryAvailable");
        const presetModes = this.getAvailablePresets();
        ...
        */
        ...
        const dishwasher = this.accessory.getService(displayName) ||
        ...
        const setDurationCharacteristic = dishwasher.getCharacteristic(this.Characteristic.SetDuration);
        try {
        ...
        const remainingDurationCharacteristic = dishwasher.getCharacteristic(this.Characteristic.RemainingDuration);
        try {
            ...
            this.client.debug("Error setting Dishwasher remainingduration properties: " + error);
        }
        dishwasher
        ...
        const optionBottlewash = this.setupService("Outlet", displayName, "optionbottlewash-223");
        optionBottlewash
            .getCharacteristic(this.Characteristic.On)
            .onGet(() => {
        ...
        const optionSilverware = this.setupService("Outlet", displayName, "optionsilverwarewash-223");
        optionSilverware
            .getCharacteristic(this.Characteristic.On)
            .onGet(() => {
            ...
            .onSet((value) => {
            this.currentSilverwareWash = value;
        ...
        const temps = washTemps.map(([item]) => this.setupGroupService("Outlet", item, "modegroup-" + item.replace(/\s+/g, "-").toLowerCase(), this.groupAccessory[0]));
        temps.forEach((service, index) => {
            service.getCharacteristic(this.Characteristic.On).onSet((value) => {
                this.client.debug("Wash Temp mode " +
                    service.displayName +
                    " set to " +
        ...
        const drymodes = heatedDrys.map(([item]) => this.setupGroupService("Outlet", item, "modegroup-" + item.replace(/\s+/g, "-").toLowerCase(), this.groupAccessory[1]));
        drymodes.forEach((service, index) => {
            service.getCharacteristic(this.Characteristic.On).onSet((value) => {
                this.client.debug("Dry Temp mode " +
                    service.displayName +
                    " set to " +
        ...
        const zones = washZones.map(([item]) => this.setupGroupService("Outlet", item, "modegroup-" + item.replace(/\s+/g, "-").toLowerCase(), this.groupAccessory[2]));
        zones.forEach((service, index) => {
            service.getCharacteristic(this.Characteristic.On).onSet((value) => {
                this.client.debug("Wash Zone mode " +
                    service.displayName +
                    " set to " +
        ...
        const presets = presetModes.map(([item]) => this.setupGroupService("Outlet", item, "modegroup-" + item.replace(/\s+/g, "-").toLowerCase(), this.groupAccessory[3]));
    ...
    async handleModeGet(v1mode) {
        ...
        for (const service of this.deviceServices) {
            if (service.serviceDeviceType === "cloud.smarthq.device.dishwasher" &&
                ...
                try {
                    const response = await this.client.getServiceDetails(this.deviceId, service.serviceId);
                    this.client.debug("================ Response from getServiceDetails for mode get: " +
    ...
    async handleRemainingTimeGet() {
        ...
        for (const service of this.deviceServices) {
            if (service.serviceDeviceType === "cloud.smarthq.device.dishwasher" &&
                ...
                this.totalSeconds = service.state?.secondsRemaining || 0;
                break;
            }
        }
        return this.totalSeconds;
    ...
    async getCyclePct() {
        ...
        const pctDone = 100 - Math.round((this.timeRemainingFromWebSocket / this.totalSeconds) * 100) || 0;
    ...
    getAvailableItemsByType(availableType) {
        let itemsAvailable = [];
        for (const service of this.deviceServices) {
            if (service.serviceDeviceType === "cloud.smarthq.device.dishwasher" &&
                ...
                switch (availableType) {
                    case "washTempAvailable":
                        itemsAvailable = service.config?.washTempAvailable || [];
                        break;
                    case "washZoneAvailable":
                        itemsAvailable = service.config?.washZoneAvailable || [];
                        break;
                    case "heatedDryAvailable":
                        itemsAvailable = service.config?.heatedDryAvailable || [];
                        break;
                    default:
                        this.client.debug("Unknown available type: " + availableType);
                }
                this.currentbottleWash = service.state?.bottleWash || false;
                this.currentSilverwareWash = service.state?.silverwarewash || false;
                this.currentSteam = service.state?.silverwarewash || false;
                const itemNames = itemsAvailable.map((item) => this.getLastElementAndCapitalize(item, "."));
                return itemNames;
            }
        }
    ...
    getLastElementAndCapitalize(str, delimiter) {
