...
export class SabbathMode {
	...
	constructor(
		...
		if (!this.platform.config.addDwSabbath) {
			// If user has not enabled Sabbath Mode switch, then don't add it
			return;
		}
		...
		if (!hasSabbathMode) {
			this.platform.log.warn(
				'[SmartHq] No supported Sabbath Mode service found for device: ' +
			...
			if (
				...
				try {
					...
					if (response?.state?.on == null) {
						this.client.debug(
							'No state.on returned from getSabbathMode state',
						);
						return false;
					}
