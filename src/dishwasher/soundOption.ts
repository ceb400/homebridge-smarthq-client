...
export class SoundOption {
	...
	constructor(
		...
		if (!this.platform.config.addDwSound) {
			// If user has not enabled Sound Option switch, then don't add it
			return;
		}
		...
		const dishsound = this.setupService(
			'Switch',
			displayName,
			`${this.deviceId}-sound-switch`,
		);
