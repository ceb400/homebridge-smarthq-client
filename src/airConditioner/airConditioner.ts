...
import { API, PlatformAccessory, Service, Characteristic } from 'homebridge';
...
export class AirConditioner {
	...
	constructor(
		...
		// Check if air conditioner service is excluded in config
		if (this.platform.config.excludeAirConditionerServices) {
			this.platform.log.info(
				chalk.yellow(
					`Air Conditioner service is excluded from config. Clearing old UUIDs for ${this.deviceId}`,
				),
			);
		...
		if (thermostatService?.state) {
			...
			if (thermostatService.state.coolCelsiusConverted != null) {
				this.lastActiveCelsius = thermostatService.state
					.coolCelsiusConverted as number;
			}
		...
		if (thermostatService?.config) {
			if (thermostatService.config.coolCelsiusMinimumConverted != null) {
				this.coolCelsiusMin = thermostatService.config
					.coolCelsiusMinimumConverted as number;
			}
			if (thermostatService.config.coolCelsiusMaximumConverted != null) {
				this.coolCelsiusMax = thermostatService.config
					.coolCelsiusMaximumConverted as number;
			}
		}
		...
		const supportedModes: string[] =
			(thermostatService?.config?.supportedModes as string[]) || [];
		const supportedFanSpeeds: string[] =
			(thermostatService?.config?.supportedFanSpeeds as string[]) || [];
	...
	private setupAccessories(supportedModes: string[], supportedFanSpeeds: string[]) {
		...
		for (const service of [...this.parentAccessory.services]) {
			if (oldServiceUUIDs.includes(service.UUID)) {
				this.platform.log.info(
					`Removing old cached service from parent accessory: ${service.displayName}`,
				);
		...
		const parentInfo = this.parentAccessory.getService(
		...
		this.acThermostat =
			...
			this.parentAccessory.addService(
				this.Service.HeaterCooler,
				name,
				`${this.deviceId}-ac-thermostat`,
			);
		...
		this.acThermostat
			...
			.onSet(async (value) => {
				...
				if (this.isOn) {
					...
					this.acThermostat.updateCharacteristic(
					...
					for (const [fanSpeed, service] of this.fanOutlets.entries()) {
						service.updateCharacteristic(
							this.Characteristic.On,
							this.lastActiveFanSpeedMode === fanSpeed,
						);
					}
		...
		if (this.modesAccessory) {
			const modeInfo = this.modesAccessory.getService(
				this.Service.AccessoryInformation,
			)!;
			modeInfo
				.setCharacteristic(this.Characteristic.Manufacturer, 'GE')
			...
			for (const mode of supportedModes) {
				...
				const service =
					...
					this.modesAccessory.addService(
						this.Service.Outlet,
						displayName,
						mode,
					);
			...
			for (const service of [...this.modesAccessory.services]) {
				...
				if (service.UUID === this.Service.Outlet.UUID) {
					...
					if (!activeModeServiceUUIDs.has(key)) {
						this.platform.log.info(
							`Removing stale Mode outlet service: ${service.displayName}`,
						);
		...
		if (this.fanAccessory) {
			const fanInfo = this.fanAccessory.getService(
				this.Service.AccessoryInformation,
			)!;
			fanInfo
				.setCharacteristic(this.Characteristic.Manufacturer, 'GE')
			...
			for (const service of [...this.fanAccessory.services]) {
				...
				if (service.UUID === this.Service.Outlet.UUID) {
					...
					if (!activeFanServiceUUIDs.has(key)) {
						this.platform.log.info(
							`Removing stale Fan Speed outlet service: ${service.displayName}`,
						);
			...
			for (const fanspeedMode of supportedFanSpeeds) {
				...
				service
					.getCharacteristic(this.Characteristic.On)
					.onGet(
						() => this.isOn && this.lastActiveFanSpeedMode === fanspeedMode,
					)
					.onSet(async (value) => {
						if (value) {
							...
							if (
								...
								setTimeout(() => {
									...
									const activeService = this.fanOutlets.get(
										this.lastActiveFanSpeedMode,
									);
									if (activeService) {
										activeService.updateCharacteristic(
											this.Characteristic.On,
											this.isOn,
										);
									}
	...
	private handleUpdate(message: ServiceMessage) {
		...
		if (message.domainType === 'cloud.smarthq.domain.thermostat') {
			...
			for (const [mode, service] of this.modeOutlets.entries()) {
				service.updateCharacteristic(
					this.Characteristic.On,
					this.isOn && this.lastActiveMode === mode,
				);
			}
			for (const [fanSpeed, service] of this.fanOutlets.entries()) {
				service.updateCharacteristic(
					this.Characteristic.On,
					this.isOn && this.lastActiveFanSpeedMode === fanSpeed,
				);
			}
	...
	private findService(serviceType: string, domainType: string) {
		return this.deviceServices.find(
			(service) =>
				service.serviceType === serviceType && service.domainType === domainType,
		);
	}
	...
	private addDebugState(serviceType: string): void {
		...
		debugState
			.getCharacteristic(this.Characteristic.On)
			.onGet(() => {
				return debugState.getCharacteristic(this.Characteristic.On)
					.value as boolean;
			})
			.onSet(async (value) => {
				if (value) {
