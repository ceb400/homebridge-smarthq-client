...
export class Dehumidifier {
	...
	private supportedFanSpeeds: string[] = [
		this.FAN_LOW,
		this.FAN_HIGH,
		this.FAN_SMART_DRY,
	];
	...
	constructor(
		...
		if (this.platform.config.excludeDehumidifierServices) {
			this.platform.log.info(
				chalk.yellow(
					`Dehumidifier service is excluded from config. Clearing old UUIDs for ${this.deviceId}`,
				),
			);
		...
		if (filter?.state?.on != null)
			this.filterNeedsCleaning = filter.state.on as boolean;
	...
	private setupServices() {
		...
		info.setCharacteristic(this.Characteristic.Manufacturer, 'GE')
			.setCharacteristic(
		...
		this.dhum
			...
			.onSet(async (value) => {
				...
				const command: Record<string, unknown> = {
					on: this.isOn,
					commandType: this.CMD_SET,
				};
				if (this.isOn) {
					command.mode =
						this.lastActiveMode === this.MODE_OFF
							? this.MODE_ON
							: this.lastActiveMode;
				}
	...
	private async sendCommand(cmdBody: SendCommandRequest) {
		try {
			...
			this.platform.log.error(
				chalk.red(`Dehumidifier sendCommand failed: ${message}`),
			);
			return false;
		}
	}
