...
export class EnergySensor {
	...
	constructor(
		...
		for (const service of deviceServices) {
			if (
				service.serviceDeviceType === 'cloud.smarthq.device.meter' &&
		...
		if (!hasEnergyMeter) {
			this.client.debug(
				'No supported Energy Meter service found for device: ' +
		...
		const energyCurrent = energySensor.getCharacteristic(
			...
			if (
				...
				try {
					...
					if (response?.state?.meterValue == null) {
						this.client.debug(
							'No state.meterValue returned from getEnergyChange state',
						);
						return false;
					}
