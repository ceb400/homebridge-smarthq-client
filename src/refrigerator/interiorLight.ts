...
export class InteriorLight {
	...
	constructor(
		...
		if (!hasInteriorLight) {
			this.client.debug(
				'No supported Interior Light service found for device: ' +
		...
		refrigeratorLight.setCharacteristic(
			...
			if (
				...
				try {
					...
					if (response?.state?.value == null) {
						this.client.debug(
							'No state.value returned from getFridgeBackLight state',
						);
						return false;
					}
					...
					this.client.debug(
						'Error getting Refrigerator Back Light value: ' + error,
					);
				}
			}
		}
