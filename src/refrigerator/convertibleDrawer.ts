...
export class ConvertibleDrawer {
	...
	constructor(
		...
		if (!hasConvertibleDrawer) {
			this.client.debug(
				'No supported Convertible Drawer service found for device: ' +
		...
		const convertibleDrawerMeat =
			this.accessory.getService(displayName) ||
			this.accessory.addService(
				this.Service.Switch,
				displayName,
				`${this.deviceId}-mode-3`,
			);
		convertibleDrawerMeat.setCharacteristic(this.Characteristic.Name, displayName);
		convertibleDrawerMeat.addOptionalCharacteristic(
			this.Characteristic.ConfiguredName,
		);
		convertibleDrawerMeat.setCharacteristic(
			this.Characteristic.ConfiguredName,
			displayName,
		);
		displayName = 'Drawer Beverages';

		const convertibleDrawerBeverages =
			this.accessory.getService(displayName) ||
			this.accessory.addService(
				this.Service.Switch,
				displayName,
				`${this.deviceId}-mode-4`,
			);
		convertibleDrawerBeverages.setCharacteristic(
			this.Characteristic.Name,
			displayName,
		);
		convertibleDrawerBeverages.addOptionalCharacteristic(
			this.Characteristic.ConfiguredName,
		);
		convertibleDrawerBeverages.setCharacteristic(
			this.Characteristic.ConfiguredName,
		...
		const drawerTemperature =
			this.accessory.getService(displayName) ||
			this.accessory.addService(
				this.Service.TemperatureSensor,
				displayName,
				`${this.deviceId}-temp-1`,
			);
		...
		drawerTemperature.setCharacteristic(
			this.Characteristic.ConfiguredName,
			displayName,
		);
	...
	async getDrawerTemperature(): Promise<CharacteristicValue> {
			...
			if (
				...
				try {
					...
					if (response == null || !response?.state?.mode) {
						this.client.debug(
							'No mode returned from getDrawerTemperature state',
						);
		...
		for (const service of this.deviceServices) {
			if (
				service.serviceDeviceType ===
					`cloud.smarthq.device.refrigerator.convertibledrawer.${mode}` &&
				...
				try {
					...
					if (response == null || !response?.state?.celsiusConverted) {
						this.client.debug(
							'No celsiusConverted returned from getDrawerTemperature state',
						);
						return false;
					}
		...
		for (const service of this.deviceServices) {
			if (
				...
				try {
					...
					if (response == null || !response?.state?.mode) {
						this.client.debug(
							'No mode returned from getConvertibleDrawerSnacks state',
						);
						return false;
					}
					...
					this.client.debug(
						'Error getting Convertible Drawer Snacks state: ' + error,
					);
					return false;
				}
			}
		...
		for (const service of this.deviceServices) {
			if (
				...
				try {
					...
					if (response == null || !response?.state?.mode) {
						this.client.debug(
							'No mode returned from getConvertibleDrawerMeat state',
						);
						return false;
					}
					...
					this.client.debug(
						'Error getting Convertible Drawer Meat state: ' + error,
					);
					return false;
				}
			}
		...
		for (const service of this.deviceServices) {
			if (
				...
				try {
					...
					if (response == null || !response?.state?.mode) {
						this.client.debug(
							'No mode returned from getConvertibleDrawerBeverages state',
						);
						return false;
					}
					...
					this.client.debug(
						'Error getting Convertible Drawer Beverages state: ' + error,
					);
					return false;
				}
			}
		...
		for (const service of this.deviceServices) {
			if (
				...
				try {
					...
					if (response == null || !response?.state?.mode) {
						this.client.debug(
							'No mode returned from getConvertibleDrawerWine state',
						);
						return false;
					}
					...
					this.client.debug(
						'Error getting Convertible Drawer Wine state: ' + error,
					);
					return false;
				}
			}
	...
	async setConvertibleDrawerMeat(value: CharacteristicValue) {
		if (value === true) {
			...
			try {
				...
				this.client.debug(
					'Error sending setConvertibleDrawerMeat command: ' + error,
				);
			}
		}
	}
	...
	async setConvertibleDrawerBeverages(value: CharacteristicValue) {
		if (value === true) {
			...
			try {
				...
				this.client.debug(
					'Error sending setConvertibleDrawerBeverages command: ' + error,
				);
			}
		}
	}
	...
	async setConvertibleDrawerSnacks(value: CharacteristicValue) {
		if (value === true) {
			...
			try {
				...
				this.client.debug(
					'Error sending setConvertibleDrawerSnacks command: ' + error,
				);
			}
		}
	}
	...
	async setConvertibleDrawerWine(value: CharacteristicValue) {
		if (value === true) {
			...
			try {
				...
				this.client.debug(
					'Error sending setConvertibleDrawerWine command: ' + error,
				);
			}
		}
	}
