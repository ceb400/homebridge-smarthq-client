...
export class Freezer {
	...
	constructor(
		...
		freezerThermostat.setCharacteristic(
			this.Characteristic.ConfiguredName,
			displayName,
		);

		const currentHeatCoolCharacteristicFreezer = freezerThermostat.getCharacteristic(
			this.Characteristic.CurrentHeatingCoolingState,
		);
		const targetHeatCoolCharacteristicFreezer = freezerThermostat.getCharacteristic(
			this.Characteristic.TargetHeatingCoolingState,
		);
		const currentTempCharacteristicFreezer = freezerThermostat.getCharacteristic(
			this.Characteristic.CurrentTemperature,
		);
		const targetTempCharacteristicFreezer = freezerThermostat.getCharacteristic(
		...
		try {
			...
			this.client.debug(
				'Error setting Freezer Current Temperature properties: ' + error,
			);
		}
		//=====================================================================================
		...
		try {
			...
			this.client.debug(
				'Error setting Freezer Target Temperature properties: ' + error,
			);
		}
		...
		for (const service of this.deviceServices) {
			if (
				service.serviceDeviceType ===
					'cloud.smarthq.device.refrigerator.freezer' &&
				...
				try {
					...
					if (response?.state?.celsiusConverted == null) {
						this.client.debug(
							'No celsiusConverted returned from getFreezerTemperature state',
						);
					...
					this.client.debug(
						`error response from getFreezerTemperature: ${error}`,
					);
