...
export class WaterFilter {
	...
	constructor(
		...
		for (const service of deviceServices) {
			if (
				service.serviceDeviceType === 'cloud.smarthq.device.waterfilter' &&
		...
		if (!hasFilterMaintenance) {
			this.client.debug(
				'No supported Filter Maintenance service found for device: ' +
		...
		const refrigeratorWaterFilter =
			...
			this.accessory.addService(
				...
				displayName,
				`${this.deviceId}-filter-maintenance`,
			);
		refrigeratorWaterFilter.addOptionalCharacteristic(
			this.Characteristic.ConfiguredName,
		);
		refrigeratorWaterFilter.setCharacteristic(
			this.Characteristic.ConfiguredName,
			displayName,
		);

		// add a lightbulb service to show the filter life level as a percentage in the Home app
		const refrigeratorPseudoFilter =
			this.accessory.getService(displayName2) ||
			this.accessory.addService(
		...
		refrigeratorPseudoFilter.addOptionalCharacteristic(
			this.Characteristic.ConfiguredName,
		);
		refrigeratorPseudoFilter.setCharacteristic(
			this.Characteristic.ConfiguredName,
			displayName2,
		);
		refrigeratorPseudoFilter
			...
			if (
				...
				try {
					...
					if (response?.state?.mode == null) {
						console.debug(
							'[SmartHq] No state.mode returned from getWaterFilterChangeIndication state',
						);
						return false;
					}
					if (
						response?.state?.mode === 'cloud.smarthq.type.mode.good' ||
					...
					console.error(
						'[SmartHq] Error getting water filter change indication:',
						error,
					);
					return false;
				}
			...
			if (
				...
				try {
					...
					if (response?.state?.value == null) {
						console.debug(
							'[SmartHq] No value returned from getWaterFilterLifeLevel state',
						);
						return false;
					}
					...
					console.error(
						'[SmartHq] Error getting water filter life level:',
						error,
					);
					return false;
				}
