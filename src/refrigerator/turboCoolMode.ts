...
export class TurboCoolMode {
	...
	constructor(
		...
		if (!hasTurboCool) {
			this.client.debug(
				'No supported Turbo Cool service found for device: ' +
		...
		const turboCoolFridge =
			this.accessory.getService(displayName) ||
			this.accessory.addService(
				this.Service.Switch,
				displayName,
				`${this.deviceId}-turbo-cool-fridge`,
			);
		...
		turboCoolFridge.setCharacteristic(
		...
		const turboCoolFreezer =
			this.accessory.getService(displayName) ||
			this.accessory.addService(
				this.Service.Switch,
				displayName,
				`${this.deviceId}-turbo-cool-freezer`,
			);
		...
		turboCoolFreezer.setCharacteristic(
			this.Characteristic.ConfiguredName,
			displayName,
		);
			...
			if (
				...
				try {
					...
					if (response?.state?.on == null) {
						this.client.debug(
							'No state.on returned from getTurboCoolFridge state',
						);
						return false;
					}
		...
		for (const service of this.deviceServices) {
			if (
				...
				try {
					...
					if (response?.state?.on == null) {
						this.client.debug(
							'No state.on returned from getTurboCoolFreezer state',
						);
						return false;
					}
					isOn = response?.state?.on === true;
