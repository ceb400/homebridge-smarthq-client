...
export class TemperatureUnits {
	...
	constructor(
		...
		for (const service of deviceServices) {
			if (
				service.serviceDeviceType === 'cloud.smarthq.device.refrigerator' &&
		...
		if (!hasTemperatureUnits) {
			this.platform.log.warn(
				'[SmartHq] No supported Temperature Units service found for device: ' +
		...
		unitsFahrenheit.setCharacteristic(
			...
			if (
				...
				try {
					...
					if (response?.state?.mode == null) {
						this.client.debug(
							'No state.mode returned from getunitsCelsius state',
						);
						return false;
					}
		...
		for (const service of this.deviceServices) {
			if (
				...
				try {
					...
					if (response?.state?.mode == null) {
						this.client.debug(
							'No state.mode returned from getunitsFahrenheit state',
						);
						return false;
					}
					const units = response?.state?.mode;
